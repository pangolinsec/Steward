...
export default function App() {
  ...
  // Global Escape key handler: close topmost modal overlay
  useEffect(() => {
    const handler = (e) => {
      if (e.key === 'Escape') {
      ...
      <div className="app-layout">
        <aside className="sidebar">
          ...
          <nav className="sidebar-nav">
            ...
            <div className="nav-group">
              <NavLink
                to="/journal"
                className={({ isActive }) => `nav-group-header${isActive ? ' active' : ''}`}
                onClick={() => { if (!journalExpanded) setJournalExpanded(true); }}
              >
                ...
                Journal
                <span
                  className={`nav-group-chevron${journalExpanded ? ' expanded' : ''}`}
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); setJournalExpanded(v => !v); }}
                >&#x25B6;</span>
              </NavLink>
              {journalExpanded && (
                <div className="nav-group-children">
                  <NavLink to="/journal">Notes</NavLink>
                  <NavLink to="/session-prep">Session Prep</NavLink>
                </div>
              )}
            </div>
        ...
        <div className="main-content">
          ...
          <Routes>
            ...
            <Route path="/session-prep" element={<SessionPrepPage campaignId={activeCampaignId} />} />
