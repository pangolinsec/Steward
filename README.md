...
To get started with a pre-built campaign, import the included oneshots from the campaign switcher (top-left menu → Import):

```
sample-data/classic-oneshot.json
sample-data/full-homebrew-cyberpunk.json
```
Note: Both of these campaigns were fully generated by Claude Opus 4.6 via the MCP server. Fully model-generated worlds are not a great way to play, DMs get huge value from the world-building process. But the MCP server can be a great way to build random tables quickly, or instruct a model on an NPC and then have the model fill in the parts you don't care about (or even for two-player duets where one player is serving as DM but still wants to be surprised by some encounters or NPCs). I used Claude for full generation here just to rapidly develop sample worlds to allow users to rapidly import a oneshot and see some of the capabilities.
